...
export type ExposureScanResult = {
  domain: string;
  score: number;
  riskLevel: string;
...
export function normalizeScanDomain(input: string) {
...
function isBlockedDomain(domain: string) {
  const blockedDomains = new Set(["localhost"]);
...
export function validateScanDomain(rawDomain: string) {
  const domain = normalizeScanDomain(rawDomain);
...
export async function runExposureScan(rawDomain: string): Promise<ExposureScanResult> {
  const domain = validateScanDomain(rawDomain);
